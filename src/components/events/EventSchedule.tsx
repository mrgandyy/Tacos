            ...
            {Object.entries(groupedEvents).map(([date, dayEvents]) => (
                <div key={date} className="relative">
                    ...
                    <div className="flex items-center gap-4 mb-8">
                        ...
                        <h3 className="text-xl md:text-3xl font-black text-white uppercase tracking-tighter shadow-brand-pink drop-shadow-[0_0_10px_rgba(255,45,134,0.5)]">
                                ...
                                <div key={event.id} className="group relative">
                                    ...
                                    <Card className={cn(
                                        ...
                                        <div className="grid md:grid-cols-12 gap-0">
                                            {/* Left: Poster Image */}
                                            <div className="md:col-span-4 lg:col-span-3 relative aspect-[4/5] md:aspect-auto md:h-full min-h-[300px] border-b md:border-b-0 md:border-r border-zinc-800 bg-black">
                                                {event.image_url ? (
                                                    <Image
                                                        src={event.image_url}
                                                        alt={event.title}
                                            ...
                                            {/* Right: Content */}
                                            <div className="md:col-span-8 lg:col-span-9 p-4 md:p-6 flex flex-col h-full">
                                                {/* Header */}
                                                <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 mb-6">
                                                    <div>
                                                        <div className="flex items-center gap-2 text-xs md:text-sm text-cyber-cyan font-mono mb-2 uppercase tracking-wide">
                                                            <Clock className="w-3 h-3 md:w-4 md:h-4" />
                                                            {format(parseISO(event.start_time), 'h:mm a')} - {format(parseISO(event.end_time), 'h:mm a')} ({event.timezone || 'Local'})
                                                        </div>
                                                        <h2 className="text-2xl md:text-3xl font-black text-white mb-2 uppercase italic leading-none group-hover:text-brand-pink transition-colors">
                                                            {event.title}
                                                        </h2>
